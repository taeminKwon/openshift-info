...
"""
###############################################################################################################
## [Title]: tomcatpodchecker.py -- a Tomcat Pod Check Script
## [Original Author]: Mike Czumak (T_v3rn1x) -- @SecuritySift
##  https://github.com/sleventyeleven/linuxprivchecker/blob/master/linuxprivchecker.py
##  [Maintainer]: Taemin Kwon
##-------------------------------------------------------------------------------------------------------------
## [Details]:
## This script is to check the result value by performing structural diagnosis based on the Tomcat Pod.
##-------------------------------------------------------------------------------------------------------------
...
###############################################################################################################
TODO:
Add Connect to the openshift cluster and prepare a VM environment that can use the oc command.
"""
...
def enum_required_command(namespace, pod_name):
    """
    필수 명령어 설치 여부
    Whether required commands are installed
    ...
    :param pod_name: Pod name to check information
    :return: None
    """

    print "[*] 필수 명령어 설치 여부...\n"
    
    command_jattach = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which jattach'"
    command_jstat = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which jstat'"
    command_curl = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which curl'"
    command_ss = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which ss'"
    command_netstat = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which netstat'"
    command_lsof = "oc exec -n " + namespace + " " + pod_name + " -- /bin/bash -c 'which lsof'"

    pod_cmd_info = {
        "jattach": {"cmd": command_jattach, "msg": "jattach", "results": []},
        "jstat": {"cmd": command_jstat, "msg": "jstat", "results": []},
        "curl": {"cmd": command_curl, "msg": "curl", "results": []}
        "ss": {"cmd": command_ss, "msg": "ss", "results": []}
        "netstat": {"cmd": command_netstat, "msg": "netstat", "results": []}
        "lsof": {"cmd": command_lsof, "msg": "lsof", "results": []}
    }

    sysinfo = execute_cmd(pod_cmd_info)
    print_results(pod_cmd_info)


def enum_required_command(workload_type, workload):
    """
    로그영역 분리 여부
    Whether to separate log area
    Enumerate Basic System Information by executing simple commands than saving the results
    :param namespace: Namespace in which the pod is running
    :param pod_name: Pod name to check information
    :return: None
    ...
    local_workload_type = workload_type
...
GC 알고리즘
JVM OPTION HeapDump
JVM Explicit GC 비활성화
JDK version
GC 로그 설정
GC 로그 설정
SERVER NAME
Metaspace(JDK1.8이상 기준)
JVM OPTION urandom
autoDeploy 설정
DB PASSWORD 암호화
에러 페이지
